                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        {group.length > 1 && (
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    const pathsToSelect = group.slice(1).map(f => f.path);
                                                    const allAlreadySelected = pathsToSelect.every(p => selectedSet.has(p));
                                                    toggleBulkSelection(pathsToSelect, !allAlreadySelected);
                                                }}
                                                className={cn(
                                                    "flex items-center gap-1.5 px-2 py-1 rounded-md transition-all border",
                                                    group.slice(1).every(f => selectedSet.has(f.path))
                                                        ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-500"
                                                        : "bg-white/5 border-white/10 text-white/40 hover:bg-white/10 hover:text-white"
                                                )}
                                            >
                                                <CopyCheck className="w-3 h-3" />
                                                <span className="text-[8px] font-black uppercase tracking-widest">Select All But One</span>
                                            </button>
                                        )}
                                    </div>
                                </td>
